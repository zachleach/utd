...
The best-, average-, and worst-case for star topology with central switch is \ul{2 hops} (e.g., the hop from source to central switch, then from central switch to the destination).

The best-, average-, and worst-case for fully-interconnected network is \ul{1 hop} (e.g., the hop from source to destination).

The best-, average-, and worst-case di-directional ring is \ul{1, $n / 2$, and $n - 1$ hops}, respectively (e.g., the source and destination nodes are adjacent, the average of 1 hop and $n - 1$ hops is $~ n/2$, and at worst the message must travel across all $n - 1$ nodes, respectively).
